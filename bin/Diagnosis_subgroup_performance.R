...
model_tuned_best = Modeling_result[["models"]][[1]]
...
source("./bin/multi_glm_modeling_evaluation_update.R")
...
write.table(tuned_feature,  "glm_test_vali_prob.txt",
...
source("./bin/multi_svm_modeling_evaluation_update.R")
unique_combinations <- paste(sort(tuned_feature), collapse = "-")  # 组合并排序为字符串
...
write.table(tuned_feature,  "svm_test_vali_prob.txt",
            row.names = TRUE, col.names = NA)
...
source("./bin/multi_xgb_modeling_evaluation_update.R")
unique_combinations <- paste(sort(tuned_feature), collapse = "-")  # 组合并排序为字符串
...
write.table(tuned_feature,  "xgb_test_vali_prob.txt",
            row.names = TRUE, col.names = NA)
